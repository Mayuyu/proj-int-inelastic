...
from .spectral_mesh import SpectralMesh



class CollisionOp(object, metaclass=ABCMeta):

    def __init__(self, config, spectral_mesh, **kwargs):

        # Load configuration
        self.config = config
        self.vm = spectral_mesh

        # Load dimension
        self.dim = int(config['collision-model']['dim'])
        assert self.dim == self.vm.dim, "'spectral_mesh' has different \
    ...
    def __call__(self, input_f, heat_bath=None, device='cpu'):
        if device == 'cpu':    
            if (not self._built_cpu
                or input_f.shape != self._input_shape):
...
class FSInelasticVHSCollisionOp(CollisionOp):
    ...
    def load_parameters(self):
        ...
        self._sphr_fac = 2*math.pi**(0.5*self.dim)/math.gamma(0.5*self.dim)
        
    def _pre_fac(self, r):
        return self._sphr_fac * r**(self._gamma+self.dim-1)
    ...
    def _build_cpu(self, input_shape):
        ...
        # Compute axis
        axis = [-(i+1) for i in range(self.dim)]
        # fft0
        ...
        # Broadcast kernels
        num_extr_dim = len(input_shape) - self.dim
        self._cpu_kernels =_broadcast_kernels(
            self._cpu_kernels, self.dim, num_extr_dim)
        # Save shape
    ...
    def _build_gpu(self, input_shape):
        ...
        self._gpu_kernels =_broadcast_kernels(
            self._gpu_kernels, self.dim, num_extr_dim)
        # Save shape
        self._input_shape = input_shape
        # Set built as true
    ...
    def collide(self, input_f):
        ...
        gain_hat = self.ffts[2](
            self.iffts[2](self.kernels['exp']*f_hat)
            *input_f)

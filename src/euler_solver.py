...
class Euler2D(object):
    ...
    def set_initial(self, rho_mean, rho_var, ux=0., uy=0., T=1., dist='uniform'):
        self._solution.t = 0.0
        # Set initial data
        xx, _ = self._domain.grid.p_centers
        solution = self._solution
        solution.q[density, ...] = rho_mean + rho_var * np.random.uniform(0., 1., size=xx.shape)
        u, v = ux, uy
        solution.q[x_momentum, ...] = solution.q[density, ...] * u
        solution.q[y_momentum, ...] = solution.q[density, ...] * v
        T = T
        solution.q[energy,...] = 0.5 * solution.q[density,...] * (u**2 + v**2) + solution.q[density, ...] * T
    ...
    def plot(self):
        ...
        cs = axes.contourf(self._claw.solution.q[energy, ...] / self._claw.solution.q[density, ...] - 0.5*(self._claw.solution.q[x_momentum, ...]**2 + self._claw.solution.q[y_momentum, ...]**2))
        plt.colorbar(cs)
